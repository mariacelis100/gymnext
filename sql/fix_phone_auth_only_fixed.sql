...
-- Eliminar TODAS las versiones existentes de la función create_user_with_phone
-- cualquiera sea la firma
...
DROP FUNCTION IF EXISTS public.create_user_with_phone(text, text, text, text, text, timestamp, boolean, boolean);
DROP FUNCTION IF EXISTS public.create_user_with_phone(text, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.create_user_with_phone(text, text, text, text, text);
DROP FUNCTION IF EXISTS public.create_user_with_phone(text, text, text, text);

-- Alternativamente, podemos usar una nueva función con un nombre diferente para evitar conflictos
CREATE OR REPLACE FUNCTION public.create_user_with_phone_v2(
  p_phone TEXT,
...
GRANT EXECUTE ON FUNCTION public.create_user_with_phone_v2 TO anon, authenticated, service_role;
